...
impl Node {
    ...
    fn get_neighbors(&self, grid: &[Vec<bool>]) -> Vec<Node> {
        DIRECTIONS
            .iter()
            .filter_map(|dir| {
                let new_point = add_points(self.point, *dir);
                if in_bounds(grid, &new_point) && grid[new_point.0 as usize][new_point.1 as usize] {
                    Some(new_point)
                } else {
                    None
                }
...
fn solve(walls: &[Point], grid_size: (isize, isize)) -> Point {
    // need to implement a binary search through the search space to find the partition index
    let mut grid = vec![vec![false; grid_size.1 as usize + 1]; grid_size.0 as usize + 1];
    let mut low = 0;
    let mut high = walls.len() - 1;
    while low <= high {
        let mid = ((high - low) / 2) + low;
        let point_set = walls[0..=mid].iter().cloned().collect();

        if is_solvable(&point_set, &mut grid) {
            low = mid + 1;
        } else {
            high = mid - 1;
        }
...
fn is_solvable(walls: &HashSet<Point>, grid: &mut [Vec<bool>]) -> bool {
    grid.iter_mut().enumerate().for_each(|(i, row)| {
        row.iter_mut()
            .enumerate()
            .for_each(|(j, cell)| *cell = !walls.contains(&(i as isize, j as isize)))
    });
    // dbg!(&grid);
    ...
    let start_point = (0, 0);
    let end_point = ((grid.len() - 1) as isize, (grid[0].len() - 1) as isize);
...
fn print_grid_seen(grid: &[Vec<bool>], seen: &HashSet<Point>) -> String {
    grid.iter()
        .enumerate()
        .map(|(i, row)| {
            row.iter()
...
2,0"
    ...
    #[test]
    fn test_sample_1() {
        let (_, walls) = parse_walls(input_fixture()).unwrap();
        let res = solve(&walls, (6, 6));
        assert_eq!((6, 1), res);
    }
    ...
    fn test_solvable() {
        ...
        let (_, walls) = parse_walls(input_fixture()).unwrap();
        let mut grid = vec![vec![false; grid_size.1 as usize + 1]; grid_size.0 as usize + 1];
        let point_set = walls[0..=12].iter().cloned().collect();
        assert!(is_solvable(&point_set, &mut grid));
    }
}
