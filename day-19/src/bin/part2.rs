...
fn solve(patterns: &HashSet<&str>, designs: Vec<&str>) -> usize {
    ...
    designs
        .into_iter()
        .map(|design| {
            count_design_solutions(design, patterns, &mut seen_designs, max_pattern_length)
        })
        .sum()
}

fn count_design_solutions<'a>(
    design: &'a str,
    patterns: &HashSet<&str>,
    seen_designs: &mut HashMap<&'a str, usize>,
        ...
        None => {
            ...
            for i in 1..=design.len().min(max_pattern_len) {
                ...
                if patterns.contains(sub_design) {
                    count +=
                        count_design_solutions(remaining, patterns, seen_designs, max_pattern_len);
                }
            }
            seen_designs.insert(design, count);
            count
        }
    }
}
...
bbrgwb"
    ...
    fn test_sample() {
        let input = input();
        let (_, (patterns, designs)) = parse_input(input).unwrap();
        assert_eq!(16, solve(&patterns, designs));
    }
}
